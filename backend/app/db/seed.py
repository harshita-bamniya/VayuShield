...
CITY_CONFIGS = [
    {
        "id": DELHI_CITY_ID, "name": "Delhi", "state": "Delhi",
        "lat": 28.6139, "lon": 77.2090,
        "stations": [
            {"id": STATION_AV_ID,  "code": "DPCC_DWARKA_SEC8",  "name": "Dwarka Sector 8",       "coords": [77.0460, 28.5921]},
    ...
    {
        "id": MUMBAI_CITY_ID, "name": "Mumbai", "state": "Maharashtra",
    ...
    {
        "id": CHENNAI_CITY_ID, "name": "Chennai", "state": "Tamil Nadu",
    ...
    {
        "id": KOLKATA_CITY_ID, "name": "Kolkata", "state": "West Bengal",
    ...
    {
        "id": PUNE_CITY_ID, "name": "Pune", "state": "Maharashtra",
...
async def _seed_delhi_emission_sources(session) -> None:
    ...
    sources = [
        {"id": "f1a2b3c4-d5e6-7890-abcd-ef1234567801", "name": "Anand Vihar Bus Depot",           "type": "vehicular",    "coords": [77.3120, 28.6450], "permit_status": "active"},
        {"id": "f2a2b3c4-d5e6-7890-abcd-ef1234567802", "name": "Delhi Thermal Power Station",      "type": "industrial",   "coords": [77.2800, 28.6200], "permit_status": "active"},
        {"id": "f3a2b3c4-d5e6-7890-abcd-ef1234567803", "name": "Ashram Chowk Construction Site",   "type": "construction", "coords": [77.2490, 28.5700], "permit_status": "pending"},
        {"id": "f4a2b3c4-d5e6-7890-abcd-ef1234567804", "name": "Haryana Border Stubble Burning Zone","type": "agricultural","coords": [76.9500, 28.7500], "permit_status": "expired"},
    ]
    for src in sources:
        await session.execute(
            text(
...
async def _seed_attribution_alerts(session) -> None:
    ...
    alerts = [
        {
            "id": str(uuid.uuid4()), "city_id": DELHI_CITY_ID,
            "alert_level": "poor",      "threshold": 200, "aqi_value": 245,
            "station_id": STATION_AV_ID,  "dominant_source": "vehicular",
            "triggered_at": now - timedelta(days=3, hours=8),
            "resolved_at":  now - timedelta(days=3, hours=2), "is_active": False,
        },
        {
            "id": str(uuid.uuid4()), "city_id": DELHI_CITY_ID,
            "alert_level": "very_poor", "threshold": 300, "aqi_value": 342,
            "station_id": STATION_ITO_ID, "dominant_source": "industrial",
            "triggered_at": now - timedelta(days=1, hours=10),
            "resolved_at":  now - timedelta(days=1, hours=3),  "is_active": False,
        },
        {
            "id": str(uuid.uuid4()), "city_id": DELHI_CITY_ID,
            "alert_level": "poor",      "threshold": 200, "aqi_value": 218,
            "station_id": STATION_AV_ID,  "dominant_source": "vehicular",
...
async def _seed_enforcement_queue(session) -> None:
    ...
    sources = [
        ...
        ("f2a2b3c4-d5e6-7890-abcd-ef1234567802", "industrial",   "active",  "Delhi Thermal Power Station"),
        ("f3a2b3c4-d5e6-7890-abcd-ef1234567803", "construction", "pending", "Ashram Chowk Construction Site"),
        ("f4a2b3c4-d5e6-7890-abcd-ef1234567804", "agricultural", "expired", "Haryana Border Stubble Burning Zone"),
    ]
    permit_weights = {"expired": 1.0, "pending": 0.6, "active": 0.2}
    source_attr    = {"vehicular": 0.35, "industrial": 0.30, "construction": 0.15, "agricultural": 0.20}

    for src_id, src_type, permit_status, src_name in sources:
        attr_w   = source_attr.get(src_type, 0.1)
        score    = round(0.35 * attr_w + 0.30 * 0.40 + 0.20 * permit_weights.get(permit_status, 0.5) + 0.15 * 1.0, 4)
        brief    = (
        ...
        await session.execute(
            ...
            {"id": str(uuid.uuid4()), "city_id": DELHI_CITY_ID, "src_id": src_id, "score": score, "brief": brief},
        )
    await session.commit()
...
async def _seed_advisories(session) -> None:
    ...
    levels = [
        ("Good",       "vehicular",    35,  "Air quality is Good — safe for all outdoor activities.",                                                                                                                                                              "वायु गुणवत्ता अच्छी है — सभी बाहरी गतिविधियाँ सुरक्षित हैं।"),
        ("Satisfactory","vehicular",   75,  "Air quality is Satisfactory — sensitive individuals may experience minor discomfort.",                                                                                                                                 "वायु गुणवत्ता संतोषजनक है — संवेदनशील व्यक्तियों को हल्की परेशानी हो सकती है।"),
        ("Moderate",   "industrial",   150, "Air quality is Moderate — prolonged outdoor exertion may cause discomfort.",                                                                                                                                          "वायु गुणवत्ता मध्यम है — लंबे समय तक बाहर परिश्रम से परेशानी हो सकती है।"),
        ("Poor",       "vehicular",    245, "Current AQI is 245 (Poor). Avoid prolonged outdoor activity. Wear N95 mask if going out. People with respiratory conditions should stay indoors.",                                                                    "वर्तमान AQI 245 (खराब) है। लंबे समय तक बाहर न रहें। N95 मास्क पहनें। श्वसन रोगी घर में रहें।"),
        ("Very Poor",  "industrial",   340, "Air quality is Very Poor (AQI 340). Avoid all outdoor activities. Keep windows closed. Use air purifiers indoors. Children and elderly must stay indoors.",                                                           "वायु गुणवत्ता बहुत खराब (AQI 340)। सभी बाहरी गतिविधियाँ बंद रखें। खिड़कियाँ बंद रखें। बच्चे और बुजुर्ग घर में रहें।"),
        ("Severe",     "agricultural", 430, "Severe air quality emergency (AQI 430) — stubble burning. Stay indoors with windows sealed. Seek medical attention if breathing difficulty occurs. All outdoor events suspended.",                                     "गंभीर वायु गुणवत्ता आपातकाल (AQI 430)। खिड़कियाँ बंद कर घर में रहें। सांस में कठिनाई हो तो तुरंत चिकित्सा लें। सभी बाहरी कार्यक्रम निलंबित।"),
    ]

    advisories = []
    for level, source, _, body_en, body_hi in levels:
        advisories += [
            {"id": str(uuid.uuid4()), "lang": "en", "title": f"Air Quality Advisory — {level}", "body": body_en, "level": level, "source": source},
            {"id": str(uuid.uuid4()), "lang": "hi", "title": f"वायु गुणवत्ता सलाह — {level}",   "body": body_hi, "level": level, "source": source},
        ]
    ...
    for adv in advisories:
        await session.execute(
            ...
            {"id": adv["id"], "city_id": DELHI_CITY_ID, "lang": adv["lang"],
             "title": adv["title"], "body": adv["body"], "level": adv["level"], "source": adv["source"]},
        )
    await session.commit()
